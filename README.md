...
> *"110 composites the language doesn't already ship. Cross-checked against `%b` — zero overlap."*
...
| `stryke-utils` (opt-in, 110 fns) | path-aware variants, n-ary wrappers, regex composites, the long tail | `deep_merge_all`, `parse_duration`, `compound_ext`, `round_to_multiple`, `pad_center`, `escape_shell`, `mask_middle`, `unwrap`, `windows`, `difference`/`intersection`/`union` |
...
| List   | `use Utils::List`   | 11 | `difference` &middot; `intersection` &middot; `union` &middot; `windows` &middot; `cartesian` &middot; `rle_encode` &middot; `rle_decode` &middot; `top_n` &middot; `bottom_n` &middot; `count_where` &middot; `transpose` |
...
110 functions total. Every sublibrary stands alone — no FFI, no required
