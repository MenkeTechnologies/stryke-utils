...
| List   | `use Utils::List`   | 12 | `difference` &middot; `intersection` &middot; `union` &middot; `windows` &middot; `chunk` &middot; `cartesian` &middot; `rle_encode` &middot; `rle_decode` &middot; `top_n` &middot; `bottom_n` &middot; `count_where` &middot; `transpose` |
...
113 functions total. Every sublibrary stands alone — no FFI, no required
