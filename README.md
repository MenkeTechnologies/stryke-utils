...
> *"123 composites the language doesn't already ship. Cross-checked against `%b` — zero overlap."*
...
| `stryke-utils` (opt-in, 123 fns) | path-aware variants, n-ary wrappers, regex composites, the long tail | `deep_merge_all`, `parse_duration`, `compound_ext`, `round_to_multiple`, `pad_center`, `escape_shell`, `mask_middle`, `unwrap`, `windows`, `difference`/`intersection`/`union` |
...
123 functions total. Every sublibrary stands alone — no FFI, no required
